...
const KidsRight = ({ kidsFilter, setCounted, setFavProduct, setFavProductImages}) => {
  ...
  useEffect(() => {
    ...
    products.filter(product => product.productGender.includes("Kids")).forEach((product) => {
  ...
  products.filter(product => product.productGender.includes("Kids")).forEach(() => {
    countProduct++;
  })
  ...
  return (
    <div className="menRight">
      ...
      {
        kidProducts.slice(0, showProducts).filter(product => {
          return (
          product.productType == kidsFilter[0] ||
          product.productType == kidsFilter[1] ||
          product.productType == kidsFilter[2] ||
          product.productType == kidsFilter[3] 
        )}
          ).map((product, index) => (
