...
const MenRight = ({ setCounted, setFavProduct, setFavProductImages}) => {
  ...
  useEffect(() => {
    ...
    products.filter(product => product.productGender.includes("Men")).forEach((product) => {
  ...
  products.filter(product => product.productGender.includes("Men")).forEach(() => {
    countProduct++;
  })
  ...
  return (
    <div className="menRight">
      ...
      {
        products.filter(product => product.productGender.includes('Men')).slice(0, showProducts).map((product, index) => (
